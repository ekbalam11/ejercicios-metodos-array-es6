...
let nuevosAnimales = animales.map((x) => x.toUpperCase(x));
...
let pacientesVisitados = pacientes.map((x) => x.ultimaVisita = new Date().toLocaleString());
...
let pacientesAvisos = pacientes.map(x => {
    return {
        ...x,
        avisos: x.ultimaVisitaPagada ? "" : "moroso"
    }
})
...
let pacientesLista = pacientes.map(pacientes => {
    return [
        tag: "<li>"pacientes.paciente"</li>"
    ]
}) // TODO
